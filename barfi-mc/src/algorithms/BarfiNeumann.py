...
class BarfiNeumann(Agent):
    def __init__(self, config):
        ...
        self.counter = 0
        self.inner_mul = self.config.inner_mul #5      # Delta times this is the number of inner optim steps
        self.first_mul = self.config.first_mul    # For first inner optim, these many additional steps are taken
    ...
    def optimize(self):
        ...
        if self.first_time:
            ...
            negJ_phi = - self.get_policy_loss(s, a, beta, r, aux_r, mask, alignment=True,
                                              ...
                                              naive=True)  # Returns the negative of J_phi
    ...
    def outer_Neumann(self, batch_size):
        ...
        outer_policy_grad = autograd.grad(Jp, self.actor.parameters(), retain_graph=False) # - \delta_\theta Jp
        ...
        inner_policy_grad = autograd.grad(Jphi, self.actor.parameters(), create_graph=True, retain_graph=True) # - \delta_\theta Jphi
        ...
        reward_grad =  autograd.grad([*inner_policy_grad, reward_L2], self.align_rewards.parameters(), grad_outputs=[*self.policy_grad_buffer2, None], retain_graph=True)
