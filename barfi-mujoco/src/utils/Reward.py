...
class Alignment_Reward(Base_Reward):
    def __init__(self, state_dim, config):
        ...
        # have a bias unit
        self.bias = nn.Linear(1, 3, bias=False)
        # self.bias.weight.data.ones_()
        self.bias.weight.data.fill_(1.0)
        # self.bias.weight.data.uniform_(0, 0.1)
    ...
    def forward(self, x, r, aux_r):
        x = self.bias(torch.ones(x.size(0), 1))
        # x = self.bias(x)
        x1 = x[:, 0:1]
        x2 = x[:, 1:2]
        x3 = x[:, 2:3]
        return x1-1 + r + x3 * aux_r # start with 0 bias
    ...
    def forward_outputs(self, x):
        with torch.no_grad():
            ...
            # return torch.mean(torch.abs(self.bias.weight.data[0])), torch.mean(torch.abs(self.bias.weight.data[2]))
