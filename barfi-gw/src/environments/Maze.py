...
## This code re-uses an existing continuous state, discrete action maze
## and tries to simulate an _almost_ NxN gridworld, where the agent starts at bootom left
## and has a goal on top right.
...
class Maze(object):
    ...
    def get_aux_rewards(self, pos, good=True, zero = True):
        # return 0
        # good = True
        if not zero:
            if good:
    ...
    def get_goal_rewards(self, pos):
        for key, val in self.reward_states.items():
            region, reward = val
            if reward and self.in_region(pos, region):
                # @  , commenting the below part to let accumulating reward near the goal state, this is for the purpose of plotting
