'''
This code will optimize with respect the aux return and print according to that
'''
...
if len(sys.argv) < 2:
    print("usage : python analysis/learning_curve_aux_return.py legend(y/n) <list of json files>")
    exit()
...
key_to_plot = 'aux_returns' # the key to plot the data

fig, axs = plt.subplots(1, figsize = (6, 4 ), dpi = 300)
for en, js in enumerate(json_handles):
    run, param , data = find_best(js, data = 'aux_returns', metric = 'auc')
    print(data.keys())
    print(param)
    agent = param['agent']
...
get_experiment_name = 'aux_return'
